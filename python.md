# オブジェクト指向の基礎（Python版）— メモリの視点から徹底解説
...
依存を注入する形にすると、「結局どこかで `EmailSender()` するのでは？」という疑問が出る。答えは **「アプリの一番外側（エントリポイント）でまとめて生成し、必要なクラスに配って回る」**。この「組み立て場所」を **Composition Root（コンポジションルート）** と呼ぶ。
