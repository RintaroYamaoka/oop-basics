...
```
 ...
 │  ヒープ領域                               │
 ├──────────────────────────────────────────┤
 │  静的領域   … static変数・定数（プログラム中ずっと存在）│
 ├──────────────────────────────────────────┤
 │  コード領域 … 命令そのもの（メソッドの本体はここ）      │
 └──────────────────────────────────────────┘
  低位アドレス
```
...
そして**関数を呼ぶと上に積まれ、関数が終わると上から外れる**。この「後に積んだものから先に外す」順序を **LIFO（Last In, First Out）** という。

```
 ① main() 実行中        ② notify() を呼んだ      ③ さらに deposit() を呼んだ
                                                  ┌─────────────┐ ← 先端(top)
                                                  │  deposit()  │
                        ┌─────────────┐           ├─────────────┤
                        │  notify()   │           │  notify()   │
 ...
 ④ deposit() が終了      ⑤ notify() が終了        ⑥ すべて戻った
                        ┌─────────────┐
                        │  notify()   │
   ┌─────────────┐      ├─────────────┤           ┌─────────────┐
   │  main()     │      │  main()     │           │  main()     │
   └─────────────┘      └─────────────┘           └─────────────┘
   depositの箱が消えた    notifyの箱も消えた         mainだけに戻った
```
...
> ⚠️ **スタックオーバーフロー**: スタックは容量が小さい。関数が関数を呼び…と積みすぎると（例: 止まらない再帰）、スタックが天井に達して `StackOverflowError`（Java）／`RecursionError`（Python）で落ちる。これがスタックが有限であることの証拠。
...
この「ヒープの片付け」を、**Java も Python も GC（ガベージコレクション）という仕組みが自動でやってくれる**（詳しくは5章）。「もう誰からも使われていない実体」をGCが見つけて回収するので、プログラマが手で「解放」を書くことは基本ない。ここがC言語などとの大きな違い。
...
```java
account = new BankAccount("山田", 1000);   // Java
```
...
```
      スタック                         ヒープ
 ┌──────────────────┐          ┌──────────────────────────┐
 │  account         │          │  BankAccount の実体        │  ← 実体はこっち
 │  = 0x7f3a  ──────┼─────────▶│    owner   = "山田"        │     (番地 0x7f3a)
 │  （番地を持つだけ）  │          │    balance = 1000         │
 └──────────────────┘          └──────────────────────────┘
      ↑ 変数はここ                    ↑ 中身(データ)はここ
```
...
```
      スタック                    ヒープ
 ┌──────────────┐          ┌──────────────┐
 │ a = 0x7f3a ──┼─────────▶│ balance=1000 │  ← 実体①（番地 0x7f3a）
 ├──────────────┤          └──────────────┘
 │ b = 0x9c10 ──┼───┐      ┌──────────────┐
 └──────────────┘   └─────▶│ balance=5000 │  ← 実体②（番地 0x9c10）
                           └──────────────┘
```
...
```
 ...
 │ a = 0x7f3a ──┼───┐
 ├──────────────┤   ├─────▶┌──────────────┐  ← 実体は1つだけ。
 │ b = 0x7f3a ──┼───┘      │ balance=9999 │     a も b も同じ番地を指す
 └──────────────┘          └──────────────┘
```
